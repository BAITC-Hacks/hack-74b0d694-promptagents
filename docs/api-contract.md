# API и архитектура v1.1.0

React → Vite proxy `/api` → FastAPI → CSV в памяти. `catalog.py` проверяет
данные при старте; `models.py` — источник схем, `frontend/src/contracts.ts` —
TypeScript-зеркало. OpenAPI: `/openapi.json`, интерактивная документация: `/docs`.
Перезагрузка данных — перезапуск backend. БД и обязательных внешних API нет.

Pipeline: город + категория → жёсткие фильтры → ранжирование → до 3
профилей → `explain()` → карточки и диагностика. Он реализован в
`backend/app/matching.py`; вход и данные проверяются до подбора.

## Запрос

`POST /api/recommend`, `Content-Type: application/json`:

```json
{
  "city": "Алматы",
  "date": "2026-10-10",
  "event_format": "свадьба",
  "category": "Ведущий",
  "budget_kzt": 300000,
  "duration_hours": 4,
  "language": "русский"
}
```

Это пример формы запроса; проверенные демосценарии — в `docs/demo.md`. Все поля, кроме
`duration_hours`, `language`, `preferences_text`, `preferences`, обязательны. Необязательные поля можно опустить
или передать `null`. Числа — конечные JSON numbers > 0 (не строки и не boolean),
дробные значения разрешены. Пустая строка языка ошибочна. Лишние поля запрещены.

`preferences_text`: строка до 2000 символов или null; NFC/пробелы/casefold,
пустая строка становится null. `preferences`: объект criterion → value, по
умолчанию `{}`, максимум 8 критериев, null не допускается. Ключи и значения —
точные машинные коды из `GET /api/preference-options`; неизвестные или
неприменимые к выбранной категории значения дают 422. Старые запросы допустимы.

Дата — строго `YYYY-MM-DD`, существующая дата включительно от 2026-09-23 до
2026-12-31. Вне окна — 422: отсутствие дат занятости не доказывает доступность.
Строки города/категории/формата/языка: Unicode NFC, обрезка и схлопывание
пробелов, `casefold()`. Сравнение только по равенству такого ключа; без
транслитерации, синонимов и замены города. Исходные подписи каталога сохраняются.
Неизвестный непустой город или категория допустимы и ведут к `category_absent`;
неизвестный формат/язык проверяется фильтром и может дать `no_match`.

## Методы и ошибки

| Метод | Результат |
| --- | --- |
| `GET /api/health` | 200: `status`, `contract_version`, `dataset_status`, `profile_count`, `data_version`, `recommendation_implemented`, `issues` |
| `GET /api/options` | 200: `cities`, `categories`, `event_formats`, `languages`, `categories_by_city`, `date_range: {min,max}`, `data_version`; 503 при ошибке данных |
| `GET /api/preference-options` | 200: `rules_version`, `categories: {"исходная категория": [{criterion,label,values:[{value,label}]}]}`; 503 при ошибке данных |
| `POST /api/recommend` | 200 с одним из трёх бизнес-исходов; 503 при ошибке данных, 422 при неверном входе |

Health — проверка процесса и каталога, не заявление о готовности подбора.
`status=degraded`, `dataset_status=missing|invalid`, `data_version=null`,
`profile_count=0` при ошибке данных; подробности в `issues`. При валидном каталоге
`status=ok`, `dataset_status=ready`, `recommendation_implemented=true`.

Options берёт только реально присутствующие значения. Массивы сортируются по
нормализованной строке, для равенства — по исходной Unicode-строке. Для одинаковых
нормализованных значений выбирается минимальная исходная строка. `categories` —
общий список; `categories_by_city` — подсказки, а не запрет отправить редкую пару.

Ошибки 422/503 имеют единую форму:

```json
{"error":{"code":"validation_error","message":"Проверьте параметры запроса","issues":[]}}
```

Коды: `validation_error` (422), `dataset_missing` / `dataset_invalid` (503).
`not_implemented` (501) остался в схеме для совместимости, но готовый маршрут
его не возвращает. Issue: `{line: number|null, id: string|null,
field: string|null, message: string}`. Синтаксическая ошибка JSON тоже 422.
Ошибки никогда не превращаются в бизнес-исход `no_match`.

## Успешный ответ recommend

| Поле | Значение |
| --- | --- |
| `status` | `matched` / `category_absent` / `no_match` |
| `message` | Понятное объяснение исхода и количества; при 1–2 объяснить дефицит, при >3 указать общее число |
| `normalized_request` | Вход после нормализации; необязательные поля явно `null`, если не заданы |
| `total_candidates` | Число профилей с городом и категорией до остальных фильтров |
| `eligible_count` | Число прошедших все условия до ограничения top-3 |
| `cards` | От 0 до 3 карточек на запрошенную дату |
| `alternative` | Отдельное предложение на другую дату или `null`; в старых ответах поле может отсутствовать |
| `diagnostics` | `filters`, `busy_profiles`, `warnings` |
| `explanation_mode` | `baseline` или `prepared` |
| `data_version` | `sha256:<64 hex>` от точных байтов исходного CSV |
| `preference_interpretation` | `preferences`, человекочитаемые `labels`, `clarification_needed`, `notes`, `interpretation_id`, `rules_version`, `mode=rules` |
| `preference_mode` | `rules` / `prepared` / `mixed`: источники признаков, реально участвовавших в оценке пожеланий |
| `feature_version` | SHA-256 снимка данных, правил и артефактов обоих видов |
| `ranking_version` | `preferences-specialization-price-id-v2` |

`category_absent`: total=eligible=0, cards=[]; `no_match`: total>0, eligible=0,
cards=[]; `matched`: eligible>0, длина cards=min(3,eligible).
Статус ответа HTTP 200 для всех трёх бизнес-исходов.

Карточка: `id`, `name` (=anon_name), `category` (выбранная категория), `city`,
`price_from_kzt`, `explanation` (1–2 предложения), `evidence` (непустой список),
`synthetic`, `city_imputed`, `price_imputed`, `origin`.
Дополнительно: `matched_preferences`, `conflicting_preferences`, `unknown_preferences`
и `score_breakdown`. Элемент списка пожеланий:
`{criterion,label,requested_value,requested_label,observed_value,observed_label,status,points,evidence}`.
status — matched/conflicting/unknown; points — 1/-1/0. Неизвестные observed_value
и observed_label — null; evidence может содержать обе явно заявленные альтернативы.
`score_breakdown`: `{preference_score,specialization,price_from_kzt,tie_break_id,criteria,ranking_version}`.
Без пожеланий списки пусты, preference_score=0. Старые учебные примеры v1.0
десериализуются с пустыми списками и null score_breakdown; runtime выдаёт полный объект.
`origin`: `source_original` — исходная анонимизированная запись;
`source_synthetic` — синтетическая запись организаторов;
`team_synthetic` — отдельная добавленная командой запись. Для исходного файла
origin вычисляется по `synthetic`, а не по имени/id. Если команда позже добавит
свои записи, нужен отдельный файл и явные метаданные происхождения; нельзя
смешивать их с исходным каталогом или менять его молча. Сейчас добавлений нет.

Метки UI: «Синтетический профиль», «Город указан при подготовке данных»,
«Цена указана при подготовке данных» при соответствующих true. Происхождение
показывать отдельно: «Исходный каталог», «Синтетика организаторов», «Добавлен командой».
Цена всегда «от … ₸»; совпадение стартовой цены не гарантирует итоговый бюджет.

## Альтернативная дата

Аддитивное расширение v1.0.0: `alternative` по умолчанию `null`. Старые ответы
без поля по-прежнему читаются; API всегда сериализует поле. Объект предложения:

| Поле | Значение |
| --- | --- |
| `date` | Другая дата `YYYY-MM-DD` в окне 2026-09-23 — 2026-12-31 включительно |
| `card` | Одна карточка той же схемы, включая непустой `card.evidence` |
| `explanation_mode` | Режим объяснения именно этой карточки: baseline / prepared |
| `warnings` | Предупреждения при формировании объяснения предложения; обычно `[]` |

Сначала выполняется обычный подбор на запрошенную дату. Только при `no_match`
проверяются остальные даты окна по исходному структурированному `busy_dates`.
Город, категория, бюджет, формат, язык и длительность остаются теми же, включая
нормализацию, равенство границ и семантику `max_hours=null`. Description и AI
не определяют доступность. Для `matched`, `category_absent` и при отсутствии
подходящего профиля во всём окне `alternative=null`.

Выбирается минимальное абсолютное расстояние в календарных днях. При равенстве
побеждает более поздняя дата. Сначала выбирается дата, затем одна карточка по
общей формуле `(-preference_score, -specialization, price_from_kzt, id)` на эту дату.
Пожелания и подготовленные признаки оцениваются по тем же правилам, что и для
основного подбора; карточка содержит их оценки и `score_breakdown`.

Исходные `status=no_match`, `eligible_count=0`, `cards=[]`, `message`,
`normalized_request`, `total_candidates`, `diagnostics` и `explanation_mode`
относятся исключительно к запрошенной дате. Альтернатива не добавляется в
`cards`, не меняет счётчики и не участвует в сравнении основной выдачи двух дат.
`data_version` общий: обе проверки используют один снимок CSV.

Объяснение `alternative.card` строится с альтернативной датой. Evidence содержат
исходные значения города, категорий, цены, форматов, календаря и заданных
необязательных условий; дата предложения отсутствует в `busy_dates`. Полный
список занятости сохранён, поэтому можно проверить как занятость на исходную
дату, так и доступность на предложенную. Цитаты допускаются по прежним правилам.

UI явно показывает обе даты, сохраняет нулевой результат исходного запроса
и подсвечивает только альтернативную карточку мягким синим цветом. Текстовые
подписи дублируют смысл цвета; evidence доступны через клавиатуру.

## Фильтры, диагностика и порядок

После отбора города и категории порядок фильтров фиксирован:

1. `busy`: запрошенная дата входит в `busy_dates` → исключить.
2. `budget`: `price_from_kzt > budget_kzt` → исключить.
3. `format`: формат отсутствует в `event_formats` → исключить.
4. `language`: язык задан и отсутствует в `languages` → исключить.
5. `duration`: длительность задана, `max_hours != null` и превышена → исключить.

Равенство цены и длительности границе допустимо. `max_hours=null` не исключает
кандидата и не означает «работает неограниченно долго» — присутствие не применимо.
Условия основного подбора не расширяются. Предложение другой даты возвращается
отдельно и не меняет исходный запрос или его результат.

`diagnostics.filters` всегда пять объектов в этом порядке:
`{reason, excluded_count, remaining_count}`. Считаются только впервые исключённые
на этапе профили; сумма excluded_count + eligible_count = total_candidates.
Пропущенный необязательный фильтр имеет excluded_count=0. Для category_absent —
пять нулевых этапов. При 1–2 результатах отличать маленький исходный каталог
от исключений условиями, опираясь на эти числа.

`busy_profiles`: только профили выбранных города и категории, исключённые первым
этапом, в порядке id. Объект `{id,name,date,evidence}`; evidence.field=`busy_dates`,
value содержит исходный список дат, в котором обязательно есть date.
Frontend сравнивает два ответа только при одинаковых параметрах кроме date и
одинаковых data_version, feature_version и ranking_version. Пожелания также должны
совпадать. Исчезновение само по себе не доказывает занятость:
сообщение «X занят» разрешено только при наличии X в busy_profiles нового ответа.
Иначе причина может быть сдвигом top-3. UI сравнивает ответы только при
совпадении остальных условий и версии данных.

Формула ранжирования (инженер №1):
`(-preference_score, -specialization, price_from_kzt, id)` по возрастанию;
specialization ∈ {0,1}. preference_score — сумма по явно заданным критериям:
подтверждённое соответствие +1, единственная явно описанная альтернатива −1,
неизвестные сведения или обе альтернативы 0. Все веса равны 1, повторения
одного критерия не суммируются. Без пожеланий первый компонент всегда 0.
Значение 1 только при валидном `format_specialization` для выбранного формата с
подтверждающей точной цитатой из description. Просто членство в event_formats
не добавляет балл. При отсутствии/устаревании признаков — 0 у всех.
Структурированные поля имеют приоритет над description и AI. Они не могут
расширять event_formats/languages/max_hours или менять календарь/цену. Если текст
им противоречит, противоречивая часть не может стать evidence соответствия.

id — исходная уникальная строка; сравнение Python Unicode code point, чувствительно
к регистру, без numeric/natural sort, locale или нормализации. Например, `HK-10`
предшествует `HK-2`. Версия формулы: `preferences-specialization-price-id-v2`.
Детерминизм гарантируется при одинаковых данных, артефакте признаков и конфигурации.

## Пожелания и воспроизводимость

Фиксированный словарь `preference_rules.py` описывает услуги, не личность.
Ведущий: delivery, formality, improvisation, guest_engagement, program_focus;
фотограф: photo_style, photographer_presence; флорист/декоратор: decor_style,
custom_concept; площадки: terrace, panorama, parking. Допустимые значения и
подписи доступны в preference-options. Примеры и ограничения — [preferences.md](preferences.md).

Свободный текст разбирается правилами без сети. LRU-кэш хранит неизменяемый JSON
для 512 комбинаций нормализованных категории, текста, выбранных критериев и
версии правил; interpretation_id — SHA-256 ключа. После перезапуска тот же
результат восстанавливается кодом; temperature не участвует. Форма имеет
приоритет над расходящимся текстом. Неоднозначная пара в тексте не применяется.
Нераспознанные формулировки не оцениваются, что явно отражено в notes.

Упоминания чисел, даты, города, бюджета, языка или длительности в тексте вызывают
clarification_needed: пользователь должен проверить поля формы. Подбор идёт по
этим полям, они никогда не изменяются молча. Пожелания не меняют eligible_count.

Для оценки нужны точные цитаты с проверкой контекста, отрицаний и противоречий
полям. Неизвестный навык не считается отсутствующим; ненавязчивость не доказывает
спокойствие; импровизация не доказывает экстраверсию. Обе альтернативы дают 0.
Цены, длина текста, личные сведения и реклама не дают баллы. Это эвристика
соответствия заказу, не вероятность успеха и не оценка качества человека.

`preferences.explain_preferences` формирует две фразы и evidence после фильтров,
выделяя сначала существенное противоречие, затем совпадение; остальные критерии
видны в структурированных списках. Без пожеланий работает прежний explain().
Если баллы и специализация равны, message указывает на сортировку по цене и ID.

## Offline-признаки пожеланий

`python -m scripts.extract_preferences --limit 1` создаёт отдельный файл для
каждого профиля: `data/derived/preferences/<sha256(id)>.json`. Каталог для API
настраивается PREFERENCES_PATH. Метаданные: schema_version=1.0.0,
rules_version=preferences-v1, dataset_sha256, profile_id, description_sha256,
model=openai/точный-snapshot, prompt_version, prompt_sha256, generated_at UTC.
features: `[{criterion,value,evidence_quote,source_field:"description",
evidence_type:"explicit"|"unknown",applicable_categories}]`.
Для unknown — value="unknown", evidence_quote=null; балл всегда 0.

LLM предлагает признаки, общий валидатор подтверждает их теми же правилами,
что используются без LLM. Проверяются метаданные, точный хеш промпта и каждый
признак. Ошибка файла даёт правила с предупреждением, а не отказ подбора.
Пропуски модели могут дополняться проверенными локальными признаками;
происхождение видно в evidence.source и preference_mode. Изменения артефактов
подхватываются при перезапуске backend. Профили без подтверждений остаются unknown.

CLI пропускает существующие файлы; для замены/обновления используйте --overwrite.
Вызовы только offline, таймаут 30 секунд на профиль, без retries. При ошибке
процесс завершается с кодом 2; ранее сохранённые профильные файлы остаются.
В /api/recommend нет сети и ожидания модели. Нет ключа/артефакта — работают
правила; невалидный AI-артефакт — правила с warning. [Инструкция](../data/derived/README.md).

## Объяснения и подготовленные признаки

Интерфейс №3: `explain(profile: Profile, request: RecommendRequest,
features: tuple[PreparedFeature, ...] = ()) -> Explanation` в `explanations.py`.
Возвращает `{text,evidence,mode}`. Функция реализована; без подготовленных
признаков работает в режиме baseline.
Вызывается после фильтрации; не меняет порядок, не обращается к сети. Инженер №1
передаёт только валидные признаки. Пустые features требуют работоспособного baseline.

Evidence: `{source: profile|description|prepared, field, value, quote: string|null}`.
Для profile — точное имя поля и исходное значение. Для description/prepared —
field=`description`, value=исходное описание, quote=непустая точная подстрока
описания. Подготовленный признак не является самостоятельным доказательством.
Релевантность специализации проверяется отдельно от вхождения цитаты: цитата
должна явно подтверждать формат, а не просто встречаться в описании.

Baseline: конкретные факты о параметрах заказа + релевантная отличительная цитата,
если есть; без выдумывания достоинств. Если отличительного факта нет, честное
объяснение по полям. Не выдавать baseline за вызов LLM. `explanation_mode=prepared`
только если хотя бы одна выданная карточка использует проверенный AI-признак;
иначе baseline. Для пустой основной выдачи — baseline; отдельная карточка
предложения имеет собственный `alternative.explanation_mode`.

Планируемый `data/derived/features.json`:

```json
{
  "schema_version": "1.0.0",
  "dataset_sha256": "<SHA-256 точных байтов CSV, 64 hex>",
  "model": "<провайдер/точная версия модели>",
  "prompt_version": "features-v1",
  "prompt_sha256": "<SHA-256 текста промпта, 64 hex>",
  "generated_at": "<ISO-8601 UTC>",
  "profiles": {
    "<исходный id>": [{
      "kind": "format_specialization",
      "event_format": "свадьба",
      "value": "свадьба",
      "quote": "<точная цитата, явно подтверждающая специализацию>"
    }]
  }
}
```

Это схема с плейсхолдерами, не реальные подготовленные данные. Второй kind —
`distinctive_detail`: value — краткое свойство, event_format — формат, для которого
оно релевантно; quote обязателен в обоих случаях. Без доказательства — не сохранять.
Экстрактор №3 работает отдельной CLI-командой через OpenAI Responses API и
сохраняет артефакт вне HTTP-запроса. Без ключа файл не создаётся, baseline
остаётся работоспособным. Настройка — в `data/derived/README.md`.

№1 проверяет схему, dataset_sha256, поддерживаемый prompt_version=`features-v1`,
наличие id, совпадение формата, точность и смысл цитат до использования признаков.
При отсутствии/устаревании/невалидности артефакта используется baseline,
причина попадает в diagnostics.warnings. Отсутствие AI-ключа не блокирует запрос.
Исходный CSV остаётся единственным источником жёстких условий. Признак, который
противоречит структурированным полям, отбрасывается с предупреждением.

## Проверка данных и примеры

Загрузчик использует стандартный csv.DictReader с encoding="utf-8-sig" и newline="".
categories/event_formats/languages/busy_dates разделяются `|`; флаги только
строки `True`/`False` (прочее — ошибка); price_from_kzt — int;
max_hours — число либо None при пустой ячейке; id — строка; description дословно.
В ошибке записи line — последняя физическая строка CSV-записи (описание может быть многострочным).

Загрузчик проверяет все строки, обязательные поля, строгие типы, уникальность id,
календарь, положительные числа, словари городов/категорий/языков/форматов, флаги.
Словарь категорий взят из HTML-превью; незнакомые значения требуют явной проверки,
а не молчаливого исправления. Пустой файл или плохая строка блокируют весь каталог.
Отчёт CLI показывает число записей, фактические значения с частотами, флаги,
пустые описания, несколько категорий и null max_hours. Число ≠66 — предупреждение (тестовые каталоги
допустимы), дубликаты/невалидные поля — ошибка с номером строки/id.

`docs/examples/responses.json` содержит явно учебные примеры всех трёх исходов.
Они не подключены к runtime. Реальные сценарии отдельно проверены в
`tests/test_real_demos.py` и `docs/demo.md` независимым алгоритмом, не заглушкой API.
