...
def test_alternative_uses_preference_ranking_and_full_card():
    catalog = load_catalog(ROOT / "tests/fixtures/preferences.csv")
    blocked = date(2026, 10, 11)
    catalog = replace(catalog, profiles=tuple(
        p.model_copy(update={"busy_dates": [blocked]}) for p in catalog.profiles
    ))
    request = RecommendRequest(city="Алматы", category="Ведущий", event_format="корпоратив",
                               date=blocked, budget_kzt=300000, language="русский",
                               duration_hours=6, preferences={"delivery": "calm"})
    result = recommend(catalog, request)
    expected = recommend(catalog, request.model_copy(update={"date": date(2026, 10, 12)}))
    assert result.status == "no_match" and result.cards == []
    assert result.alternative.date == date(2026, 10, 12)
    assert result.alternative.card == expected.cards[0]
    assert result.alternative.card.score_breakdown.preference_score == 1
    assert result.alternative.card.matched_preferences
    assert result.feature_version == expected.feature_version
    assert result.ranking_version == expected.ranking_version
